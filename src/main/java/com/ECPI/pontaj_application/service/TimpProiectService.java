...
public class TimpProiectService {
    ...
    public List<TimpProiect> findAllByDate(LocalDate localDate) {
        return timpProiectRepository.findAllByDate(localDate);
    }
    ...
    public List<TimpProiect> findAllByMonthAndProiectId(int month, UUID proiectId) {
        return timpProiectRepository.findAllByMonthAndProiectId(month, proiectId);
    }

    public List<TimpProiect> findAllByProiectIdAndAngajatId(UUID proiectId, UUID angajatId) {
        return timpProiectRepository.findAllByProiectIdAndAngajatId(proiectId, angajatId);
    }
    public List<TimpProiect> findAllByAngajatIdAndProiectId(UUID proiectId, UUID angajatId) {
        return timpProiectRepository.findAllByAngajatIdAndProiectId(proiectId, angajatId);
    }
            ...
            for (Map.Entry<YearMonth, Map<LocalDate, List<TimpProiect>>> monthEntry : sortedByMonthAndDate.entrySet()) {
                ...
                for (Map.Entry<LocalDate, List<TimpProiect>> dateEntry : monthEntry.getValue().entrySet()) {
                    for (TimpProiect timpProiect : dateEntry.getValue()) {
                        if (!timpProiect.getProiect().isLivrat() && !timpProiect.isMascat()) { // Check if the timpProiect is not mascat
                ...
                for (int columnIndex = 1; columnIndex <= projectSet.size() + 1; columnIndex++) {
                    ...
                    String sumFormula = "SUM(" + columnLetter + "2:" + columnLetter + rowIdx + ")";
                ...
                for (Map.Entry<LocalDate, List<TimpProiect>> dateEntry : sortedDateEntries.entrySet()) {
                    ...
                    for (int columnIndex = 1; columnIndex <= projectSet.size() + 1; columnIndex++) {
                        ...
                        if (columnIndex == colIdx) {
                            sumFormula = "SUM(" + columnLetter + "3:" + columnLetter + (rowIndex + 1) + ")"; // Adjusted range to include the second row
                        } else {
                            sumFormula = "SUM(" + columnLetter + "3:" + columnLetter + rowIndex + ")"; // Adjusted range to include the second row
                        }
    ...
    public byte[] exportTimpProiectEntitiesToExcelTotal() throws IOException {
        // Filter timpProiectList to include only timpProiects of active employees
        List<TimpProiect> timpProiectList = timpProiectRepository.findAll();
        timpProiectList = timpProiectList.stream()
                .filter(timpProiect -> !timpProiect.getAngajat().isActiv())
    ...
    public byte[] exportTimpProiectEntitiesToExcelLivrat() throws IOException {
        // Filter timpProiectList to include only timpProiects of active employees
        List<TimpProiect> timpProiectList = timpProiectRepository.findAll();
        timpProiectList = timpProiectList.stream()
                .filter(timpProiect -> !timpProiect.getAngajat().isActiv())
    ...
    public byte[] exportTimpProiectEntitiesToExcelMonths() throws IOException {
        // Filter timpProiectList to include only timpProiects of active employees
        List<TimpProiect> timpProiectList = timpProiectRepository.findAll();
        timpProiectList = timpProiectList.stream()
                .filter(timpProiect -> !timpProiect.getAngajat().isActiv())
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
            ...
            Map<YearMonth, List<TimpProiect>> groupedByMonth = timpProiectList.stream()
                    .collect(Collectors.groupingBy(t -> YearMonth.from(t.getData())));

            Map<YearMonth, List<TimpProiect>> sortedByMonth = new TreeMap<>(groupedByMonth);

            for (Map.Entry<YearMonth, List<TimpProiect>> monthEntry : sortedByMonth.entrySet()) {
                YearMonth yearMonth = monthEntry.getKey();
                String monthSheetName = yearMonth.getMonth().toString() + " " + yearMonth.getYear();
                Sheet monthSheet = workbook.createSheet(monthSheetName);
                ...
                Row monthHeaderRow = monthSheet.createRow(rowIdx++);
                Set<String> projectSet = new HashSet<>();
                for (TimpProiect timpProiect : monthEntry.getValue()) {
                    projectSet.add(String.valueOf(timpProiect.getProiect().getNrComandaInt()));
                }
    ...
    public byte[] exportTimpProiectEntitiesToExcelForMonth(int month) throws IOException {
        // Filter timpProiectList to include only timpProiects of active employees and for the specified month
        List<TimpProiect> timpProiectList = timpProiectRepository.findAll().stream()
                .filter(timpProiect -> !timpProiect.getAngajat().isActiv() &&
                        timpProiect.getData().getMonthValue() == month)
                .collect(Collectors.toList());

        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {

            // Only one sheet for the specified month
            ...
            // Create header row for the month sheet
            Row monthHeaderRow = monthSheet.createRow(0);
            Set<String> projectSet = new HashSet<>();
            for (TimpProiect timpProiect : timpProiectList) {
                if (!timpProiect.getProiect().isLivrat()) {
                    projectSet.add(String.valueOf(timpProiect.getProiect().getNrComandaInt()));
            ...
            // Populate data rows
            for (TimpProiect timpProiect : timpProiectList) {
                if (!timpProiect.getProiect().isLivrat() && !timpProiect.isMascat()) { // Exclude mascat timpProiects
                    String employeeName = timpProiect.getAngajat().fullName();
                    String project = String.valueOf(timpProiect.getProiect().getNrComandaInt());
                    float hours = timpProiect.getOre();
            ...
            // Fill in the table data for the month sheet
            int rowIdx = 1; // Start from the second row
            for (Map.Entry<String, Map<String, Float>> entry : employeeProjectHours.entrySet()) {
                Row row = monthSheet.createRow(rowIdx++);
                colIdx = 1;
                // Set employee name in the first column
    ...
    public byte[] exportTimpProiectEntitiesToExcelForDate(LocalDate date) throws IOException {
        // Filter timpProiectList to include only timpProiects of active employees and for the specified date
        List<TimpProiect> timpProiectList = timpProiectRepository.findAll().stream()
                .filter(timpProiect -> !timpProiect.getAngajat().isActiv() &&
                        timpProiect.getData().equals(date))
                .collect(Collectors.toList());

        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {

            // Only one sheet for the specified date
            Sheet dateSheet = workbook.createSheet(date.toString());
            ...
            Cell totalCell = dateHeaderRow.createCell(colIdx);
            totalCell.setCellValue("Total");
            ...
            for (TimpProiect timpProiect : timpProiectList) {
                if (!timpProiect.getProiect().isLivrat() && !timpProiect.isMascat()) { // Exclude mascat timpProiects
                    ...
                    // Skip adding hours if it's mascat
            ...
            int rowIdx = 1; // Start from the second row
            for (Map.Entry<String, Map<String, Float>> entry : employeeProjectHours.entrySet()) {
                Row row = dateSheet.createRow(rowIdx++);
                colIdx = 1;
                // Set employee name in the first column
                row.createCell(0).setCellValue(entry.getKey());
                for (String project : projectSet) {
            ...
            // Add sum formula for each column in the last row
            Row totalRowDate = dateSheet.createRow(rowIdx);
            totalRowDate.createCell(0).setCellValue("Total");
